...
def predict_record(record: dict, model_dir: str | Path = "artifacts") -> dict:
    ...
    return {
        ...
        "confidence_index": round(float(result["confidence_index"]), 4),
        "support_tier": result.get("support_tier", "unknown"),
        "exact_variant_rows": int(result.get("exact_variant_rows", 0)),
        "exact_model_rows": int(result.get("exact_model_rows", 0)),
        "usable_comp_count": int(result.get("usable_comp_count", 0)),
        "comp_quality_passed": bool(result.get("comp_quality_passed", False)),
        "blend_mode": result.get("blend_mode", "anchor_only"),
        "inspection_source": result.get("inspection_source", "missing"),
        "inspection_completeness": round(float(result.get("inspection_completeness", 0.0)), 4),
